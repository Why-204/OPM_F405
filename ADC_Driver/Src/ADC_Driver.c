...
static const uint32_t s_dma_spi2_rx_clear_flags =
    DMA_FLAG_FEIF3_7 | DMA_FLAG_DMEIF3_7 | DMA_FLAG_TEIF3_7 | DMA_FLAG_HTIF3_7 | DMA_FLAG_TCIF3_7;
...
static HAL_StatusTypeDef adc_driver_start_rxonly_dma(void)
{
    ...
    if ((hdma->Instance->CR & DMA_SxCR_EN) != 0U)
    {
        return HAL_BUSY;
    }
    ...
    hdma->Instance->PAR = (uint32_t)&s_hspi->Instance->DR;
    hdma->Instance->M0AR = (uint32_t)s_dma_frame;
    hdma->Instance->NDTR = ADC_FRAME_BYTES;
    __HAL_DMA_CLEAR_FLAG(hdma, s_dma_spi2_rx_clear_flags);
    CLEAR_BIT(hdma->Instance->CR, DMA_IT_HT);
    SET_BIT(hdma->Instance->CR, DMA_IT_TC | DMA_IT_TE | DMA_IT_DME);
