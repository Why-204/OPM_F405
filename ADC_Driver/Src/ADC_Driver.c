...
#define ADC_DMA_RX_ALL_FLAGS (DMA_FLAG_TCIF3_7 | DMA_FLAG_HTIF3_7 | DMA_FLAG_TEIF3_7 | \
                              DMA_FLAG_DMEIF3_7 | DMA_FLAG_FEIF3_7)
#define ADC_DMA_RX_ERROR_FLAGS (DMA_FLAG_TEIF3_7 | DMA_FLAG_DMEIF3_7 | DMA_FLAG_FEIF3_7)
#define ADC_DMA_TX_ALL_FLAGS (DMA_FLAG_TCIF0_4 | DMA_FLAG_HTIF0_4 | DMA_FLAG_TEIF0_4 | \
...
/* Dummy TX clocks the read; SPI2 MOSI is intentionally not routed to a GPIO. */
static uint8_t s_dma_dummy_tx[ADC_FRAME_BYTES];
static volatile uint8_t s_running;
static volatile uint8_t s_dma_busy;
static volatile uint8_t s_frame_timing_invalid;
static volatile uint8_t s_frame_notify_pending;
static volatile uint32_t s_sequence;
static volatile uint32_t s_discard_remaining;
static volatile uint32_t s_last_drdy_cycle;
static volatile uint32_t s_dma_start_cycle;
...
volatile uint32_t adc_drdy_period_cycles;
volatile uint32_t adc_dma_duration_cycles;
volatile uint32_t adc_overlap_dma_ndtr;
volatile uint32_t adc_system_core_clock_hz;
...
{
    uint8_t frame_timing_invalid = s_frame_timing_invalid;
...
static void adc_driver_prepare_dma(void)
{
    DMA_HandleTypeDef *rx_dma = s_hspi->hdmarx;
    DMA_HandleTypeDef *tx_dma = s_hspi->hdmatx;

    __HAL_DMA_DISABLE(rx_dma);
    __HAL_DMA_DISABLE(tx_dma);

    rx_dma->Instance->PAR = (uint32_t)&s_hspi->Instance->DR;
    rx_dma->Instance->M0AR = (uint32_t)s_dma_frame;
    rx_dma->Instance->NDTR = ADC_FRAME_BYTES;
    tx_dma->Instance->PAR = (uint32_t)&s_hspi->Instance->DR;
    tx_dma->Instance->M0AR = (uint32_t)s_dma_dummy_tx;
    tx_dma->Instance->NDTR = ADC_FRAME_BYTES;

    rx_dma->Instance->CR &= ~(DMA_IT_TC | DMA_IT_HT | DMA_IT_TE | DMA_IT_DME);
    rx_dma->Instance->CR |= DMA_IT_TC | DMA_IT_TE | DMA_IT_DME;
    tx_dma->Instance->CR &= ~(DMA_IT_TC | DMA_IT_HT | DMA_IT_TE | DMA_IT_DME);

    DMA1->LIFCR = ADC_DMA_RX_ALL_FLAGS;
    DMA1->HIFCR = ADC_DMA_TX_ALL_FLAGS;
    ...
    s_hspi->pTxBuffPtr = s_dma_dummy_tx;
    s_hspi->TxXferSize = ADC_FRAME_BYTES;

    SET_BIT(s_hspi->Instance->CR2, SPI_CR2_RXDMAEN);
    __HAL_SPI_ENABLE(s_hspi);
}

static HAL_StatusTypeDef adc_driver_start_dma_fast(void)
{
    DMA_HandleTypeDef *rx_dma = s_hspi->hdmarx;
    DMA_HandleTypeDef *tx_dma = s_hspi->hdmatx;

    if ((s_hspi->State != HAL_SPI_STATE_READY) ||
        ((rx_dma->Instance->CR & DMA_SxCR_EN) != 0U) ||
        ((tx_dma->Instance->CR & DMA_SxCR_EN) != 0U))
    {
        return HAL_BUSY;
    }

    __HAL_DMA_ENABLE(rx_dma);
    __HAL_DMA_ENABLE(tx_dma);
    SET_BIT(s_hspi->Instance->CR2, SPI_CR2_TXDMAEN);

    rx_dma->State = HAL_DMA_STATE_BUSY;
    rx_dma->ErrorCode = HAL_DMA_ERROR_NONE;
    tx_dma->State = HAL_DMA_STATE_BUSY;
    tx_dma->ErrorCode = HAL_DMA_ERROR_NONE;
    s_hspi->State = HAL_SPI_STATE_BUSY_TX_RX;
...
{
    if (!adc_driver_is_spi2(hspi) || (hspi->Init.Direction != SPI_DIRECTION_2LINES))
    {
        return ADC_STATUS_INVALID_PARAM;
    }
    if ((hspi->hdmarx == NULL) || (hspi->hdmatx == NULL))
    {
    ...
    s_dma_busy = 0U;
    s_frame_timing_invalid = 0U;
    s_frame_notify_pending = 0U;
    s_sequence = 0U;
    s_discard_remaining = 0U;
    s_last_drdy_cycle = 0U;
    s_dma_start_cycle = 0U;
    ...
    adc_last_spi_cr1 = 0U;
    adc_last_spi_cr2 = 0U;
    adc_drdy_period_cycles = 0U;
    adc_dma_duration_cycles = 0U;
    adc_overlap_dma_ndtr = 0U;
    adc_system_core_clock_hz = SystemCoreClock;
    ...
    memset(s_dma_dummy_tx, 0, sizeof(s_dma_dummy_tx));
    memset((void *)adc_latest_raw_bytes, 0, sizeof(adc_latest_raw_bytes));
    adc_driver_prepare_dma();
    ADC_Buffer_Reset();
...
void ADC_Driver_Stop(void)
{
    ...
    if (s_hspi != NULL)
    {
        ...
        {
            __HAL_DMA_DISABLE(s_hspi->hdmarx);
            s_hspi->hdmarx->State = HAL_DMA_STATE_READY;
        }
        if (s_hspi->hdmatx != NULL)
        {
            __HAL_DMA_DISABLE(s_hspi->hdmatx);
            s_hspi->hdmatx->State = HAL_DMA_STATE_READY;
        }
...
void ADC_Driver_DMA_RX_IRQHandler(void)
{
    ...
    if ((s_hspi == NULL) || (s_hspi->hdmarx == NULL) || (s_hspi->hdmatx == NULL))
    {
        return;
    }

    rx_dma = s_hspi->hdmarx;
    tx_dma = s_hspi->hdmatx;
    dma_status = DMA1->LISR;

    if ((dma_status & ADC_DMA_RX_ERROR_FLAGS) != 0U)
    {
        DMA1->LIFCR = ADC_DMA_RX_ALL_FLAGS;
        CLEAR_BIT(s_hspi->Instance->CR2, SPI_CR2_RXDMAEN | SPI_CR2_TXDMAEN);
        __HAL_DMA_DISABLE(rx_dma);
        ...
        rx_dma->State = HAL_DMA_STATE_READY;
        tx_dma->State = HAL_DMA_STATE_READY;
        rx_dma->ErrorCode = HAL_DMA_ERROR_TE;
        s_hspi->State = HAL_SPI_STATE_READY;
    ...
    if ((dma_status & DMA_FLAG_TCIF3_7) == 0U)
    {
        return;
    }

    adc_dma_duration_cycles = DWT->CYCCNT - s_dma_start_cycle;
    DMA1->LIFCR = ADC_DMA_RX_ALL_FLAGS;
    DMA1->HIFCR = ADC_DMA_TX_ALL_FLAGS;
    CLEAR_BIT(s_hspi->Instance->CR2, SPI_CR2_TXDMAEN);
    rx_dma->Instance->NDTR = ADC_FRAME_BYTES;
    tx_dma->Instance->NDTR = ADC_FRAME_BYTES;

    timeout = 64U;
    while ((__HAL_SPI_GET_FLAG(s_hspi, SPI_FLAG_BSY) != RESET) && (timeout > 0U))
    {
        timeout--;
    }
    ...
    rx_dma->State = HAL_DMA_STATE_READY;
    tx_dma->State = HAL_DMA_STATE_READY;
    s_hspi->RxXferCount = 0U;
    s_hspi->TxXferCount = 0U;
    s_hspi->State = HAL_SPI_STATE_READY;

    adc_driver_handle_dma_complete(s_hspi);
}
...
void ADC_Driver_EXTI_Callback(uint16_t GPIO_Pin)
{
    ...
    if (s_dma_busy != 0U)
    {
        adc_overlap_dma_ndtr = adc_driver_dma_ndtr(s_hspi);
        if (adc_overlap_dma_ndtr == 0U)
        {
            ADC_Driver_DMA_RX_IRQHandler();
        }
    }
    ...
    if (s_dma_busy != 0U)
    {
        ...
        adc_drdy_period_cycles = (s_last_drdy_cycle != 0U) ? (now - s_last_drdy_cycle) : 0U;
        s_last_drdy_cycle = now;
        s_frame_timing_invalid = 1U;
        s_spi_busy_count++;
        s_missed_drdy_count++;
        s_timing_discard_count++;
    ...
    s_dma_busy = 1U;
    s_frame_timing_invalid = 0U;
    s_dma_start_cycle = now;
    hal_status = adc_driver_start_dma_fast();
